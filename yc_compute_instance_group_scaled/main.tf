...
resource "yandex_compute_instance_group" "instance_group" {
  ...
  instance_template {
    name        = "${var.env}-s{instance.index}${var.instance_role}-${var.instance_name}"
    #platform_id = lookup(var.instance_platform,var.env)
    platform_id = var.instance_platform
    hostname    = "${var.env}-s{instance.index}${var.instance_role}-${var.instance_name}.${var.domain_fqdn}"
